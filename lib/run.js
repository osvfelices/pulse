...
if (!filePath) {
  console.error('Usage: pulse <file.pulse> [--sourcemap] [--strict-ast] [--strict-semantic] [--strict-types] [--experimental-ir]')
  console.error('')
  ...
  console.error('  --experimental-ir   Use experimental IR-based backend (instead of legacy codegen)')
  process.exit(1)
}
...
try {
  ...
  if (experimentalIR) {
    ...
    if (enableSourceMap) {
      console.warn('Warning: --sourcemap not yet supported with --experimental-ir')
    }
  } else {
    // Legacy codegen path
    if (enableSourceMap) {
