...
function generateCode(ast) {
  if (ast.kind === 'Program') {
    // No preamble needed - globals already available
...
function generateImport(stmt) {
  // Resolve relative imports to absolute based on the source file location
  let source = stmt.source
  if (source.startsWith('.') || source.startsWith('/')) {
    // Keep as-is for relative/absolute paths
    source = stmt.source
  } else if (!source.startsWith('node:')) {
    // For non-node modules, keep as-is
    source = stmt.source
  }
