...
export class Parser{
  ...
  parseStmt(){
    ...
    if(this.at('let')||this.at('const')){ const v=this.parseVar(); this.optionalSemicolon(); return v; }
    if(this.at('return')){ this.eat('return'); let e=null; if(!this.at(';')&&!this.at('}')&&this.t) e=this.parseExpr(); this.optionalSemicolon(); return {kind:'ReturnStmt',expr:e}; }
    ...
    if(this.at('throw')){ this.eat('throw'); const e=this.parseExpr(); this.optionalSemicolon(); return {kind:'ThrowStmt',expr:e}; }
    if(this.at('break')){ this.eat('break'); this.optionalSemicolon(); return {kind:'BreakStmt'}; }
    if(this.at('continue')){ this.eat('continue'); this.optionalSemicolon(); return {kind:'ContinueStmt'}; }
    if(this.at('switch')) return this.parseSwitch();
    if(this.at('class')) return this.parseClass();
    const stmt={kind:'ExprStmt',expr:this.parseExpr()}; this.optionalSemicolon(); return stmt;
  }
  ...
  parseFor(){
    ...
    if(this.at('of')){
      ...
      if (isAwait) {
        return {kind:'ForAwaitStmt',variable:init,iterable,body};
      }
    ...
    if (isAwait) {
      throw new Error('await is only valid in for-of loops');
    }
    this.eat(';');
    const test=this.parseExpr();
    this.eat(';');
  ...
  parseUnary(){
    ...
    if(this.at('spawn')||this.at('go')){
      ...
      return {kind:'SpawnExpr',argument:arg};
    }
    if(this.at('yield')){
      this.eat('yield');
      ...
      if(this.at(';')||this.at('}')||this.at(')')||this.at(',')||this.at(']')||
        ...
        return {kind:'YieldExpr',argument:null};
      }
      const arg=this.parseArrow();
      return {kind:'YieldExpr',argument:arg};
    }
    if(this.at('new')){
      this.eat('new');
  ...
  parseSelect(){
    // Parse select expression: select { case recv ch1, case send ch2 val, ... }
    // Returns SelectExpr node with cases array
    this.eat('select');
    this.eat('{');
      ...
      if(this.at('case')){
        ...
        if(this.at('ident')){
          ...
          if(opText === 'recv' || opText === 'send'){
            ...
            throw new Error('expected recv or send in select case, got ' + opText);
          }
        } else {
          throw new Error('expected recv or send in select case');
        }
        ...
        cases.push({op, channel, value});

        // Optional comma or newline between cases
        if(this.at(',')) this.eat(',');
      } else if(this.at('default')){
        // Optional default case (not implemented yet, reserved for future)
        throw new Error('default case not yet supported in select');
      } else {
        throw new Error('expected case in select block');
      }
    }
    ...
    if(cases.length === 0){
      throw new Error('select must have at least one case');
    }

    return {kind:'SelectExpr', cases};
  }
}
