 ...
 * Pulse Parser
 *
 * Transforms token stream into AST using recursive descent.
 * Uses factory functions from ast/factory.js for node construction.
 */
...
export class Parser {
  constructor(src, options = {}) {
    ...
    this.enableValidation = options.validateAST || false;
  }
  ...
  parseProgram() {
    ...
    const program = createProgram(body, this.loc(start, end));

    // Optional AST validation pass
    if (this.enableValidation) {
      const result = validateAST(program);
      if (!result.valid) {
        result.errors.forEach(err => this.reportError(err));
        this.throwErrors();
      }
    }
  ...
  parseBlockOrStatement() {
    if (this.at('{')) {
      ...
      const stmt = this.parseStmt();
      const end = this.endLoc();
      return createBlock([stmt], this.loc(start, end));
    }
  }
  ...
  parseStmt() {
    ...
    if (this.at('return')) {
      ...
      this.optionalSemicolon();
      const end = this.endLoc();
      return createReturnStmt(e, this.loc(start, end));
    }
    ...
    if (this.at('throw')) {
      ...
      this.optionalSemicolon();
      const end = this.endLoc();
      return createThrowStmt(e, this.loc(start, end));
    }
    if (this.at('break')) {
      const start = this.startLoc();
      this.eat('break');
      this.optionalSemicolon();
      const end = this.endLoc();
      return createBreakStmt(this.loc(start, end));
    }
    if (this.at('continue')) {
      const start = this.startLoc();
      this.eat('continue');
      this.optionalSemicolon();
      const end = this.endLoc();
      return createContinueStmt(this.loc(start, end));
    }
    ...
    this.optionalSemicolon();
    const end = this.endLoc();
    return createExprStmt(expr, this.loc(start, end));
  }
  ...
  parseImport() {
    ...
    if (this.at('string')) {
      ...
      this.optionalSemicolon();
      const end = this.endLoc();
      return createImportDecl(src, { sideEffect: true }, this.loc(start, end));
    }
  ...
  parseExport() {
    ...
    if (this.at('default')) {
      ...
      this.optionalSemicolon();
      const end = this.endLoc();
      return createExportDefault(expr, this.loc(start, end));
    }
    ...
    if (this.at('*')) {
      ...
      this.optionalSemicolon();
      const end = this.endLoc();
      return createExportAll(src, asName, this.loc(start, end));
    }
    ...
    if (this.at('{')) {
      ...
      if (this.at('from')) {
        ...
        this.optionalSemicolon();
        const end = this.endLoc();
        return createExportNamed(specifiers, src, this.loc(start, end));
      }

      // Local export: export { a }
      this.optionalSemicolon();
      const end = this.endLoc();
      return createExportNamed(specifiers, null, this.loc(start, end));
    }
    ...
    if (this.at('fn') || this.at('async')) {
      ...
      const end = this.endLoc();
      return createExportDecl(fn, this.loc(start, end));
    }
    if (this.at('class')) {
      const cls = this.parseClass();
      const end = this.endLoc();
      return createExportDecl(cls, this.loc(start, end));
    }
    if (this.at('const') || this.at('let')) {
      const varDecl = this.parseVar();
      this.optionalSemicolon();
      const end = this.endLoc();
      return createExportDecl(varDecl, this.loc(start, end));
    }

    const error = ParserErrors.invalidExportSyntax(this, 'expected default, *, {...}, fn, class, const, or let');
    this.reportError(error);
    this.recover();
    return createExportDecl(null, this.loc(start, this.endLoc()));
  }
  ...
  parseFn() {
    ...
    while (!this.at(')')) {
      ...
      params.push({ name: id, default: defaultValue, typeAnnotation });
      if (this.at(',')) this.eat(',');
      else break;
    }
    this.eat(')');

    // Return type annotation: fn(): Type
    let returnType = null;
    if (this.at(':')) {
      this.eat(':');
    ...
    const end = this.endLoc();
    return createFnDecl(name, params, body, isAsync, this.loc(start, end), returnType);
  }

  parseTypeAnnotation() {
    // Simple type annotation parser
    // Supports: int, float, bool, string, Channel<T>, Array<T>, etc.
    const baseType = this.eat('ident').text;
    ...
    if (this.at('<')) {
      ...
      this.eat('>');
      return { base: baseType, elementType };
    }
  ...
  parseVar() {
    ...
    if (this.at('[')) {
      ...
      while (!this.at(']')) {
        if (this.at('...')) {
          ...
          elements.push(createRestElement(name, this.loc(elemStart, elemEnd)));
          break;
        }
      ...
      const end = this.endLoc();
      const pattern = createArrayPattern(elements, this.loc(patternStart, patternEnd));
      return createVarDecl(c, pattern, init, this.loc(start, end));
    }
    ...
    if (this.at('{')) {
      ...
      const init = this.parseExpr();
      const end = this.endLoc();
      const pattern = createObjectPattern(properties, this.loc(patternStart, patternEnd));
      return createVarDecl(c, pattern, init, this.loc(start, end));
    }
    ...
    const end = this.endLoc();
    return createVarDecl(c, name, init, this.loc(start, end), typeAnnotation);
  }
  ...
  parseIf() {
    ...
    const end = this.endLoc();
    return createIfStmt(test, consequent, alternate, this.loc(start, end));
  }
  ...
  parseFor() {
    ...
    if (this.at('let') || this.at('const')) {
      ...
      const exprStart = this.startLoc();
      const expr = this.parseExpr();
      const exprEnd = this.endLoc();
      init = createExprStmt(expr, this.loc(exprStart, exprEnd));
    }
    ...
    if (this.at('of')) {
      ...
      const end = this.endLoc();
      if (isAwait) {
        return createForAwaitStmt(init, iterable, body, this.loc(start, end));
      }
      return createForOfStmt(init, iterable, body, this.loc(start, end));
    }
    ...
    if (this.at('in')) {
      ...
      const body = this.parseBlockOrStatement();
      const end = this.endLoc();
      return createForInStmt(init, object, body, this.loc(start, end));
    }
    ...
    const body = this.parseBlockOrStatement();
    const end = this.endLoc();
    return createForStmt(init, test, update, body, this.loc(start, end));
  }
  ...
  parseWhile() {
    ...
    const body = this.parseBlockOrStatement();
    const end = this.endLoc();
    return createWhileStmt(test, body, this.loc(start, end));
  }
  ...
  parseTry() {
    ...
    const end = this.endLoc();
    return createTryStmt(body, handler, finalizer, this.loc(start, end));
  }
  ...
  parseSwitch() {
    ...
    this.eat('}');
    const end = this.endLoc();
    return createSwitchStmt(discriminant, cases, this.loc(start, end));
  }
  ...
  parseClass() {
    ...
    this.eat('}');
    const end = this.endLoc();
    return createClassDecl(name, superClass, methods, this.loc(start, end));
  }
  ...
  parseContract() {
    ...
    this.eat('}');
    const end = this.endLoc();
    return createContractDecl(name, fields, this.loc(start, end));
  }
  ...
  parseView() {
    ...
    const body = this.parseBlock();
    const end = this.endLoc();
    return createViewDecl(name, params, body, this.loc(start, end));
  }
  ...
  parseBlock() {
    ...
    this.eat('}');
    const end = this.endLoc();
    return createBlock(st, this.loc(start, end));
  }
  ...
  parseArrow() {
    ...
    if (this.at('ident')) {
      ...
      if (this.at('=>')) {
        ...
        const end = this.endLoc();
        return createArrowFn([param], body, isAsync, this.loc(start, end));
      }
    ...
    if (this.at('(')) {
      ...
      if (isArrowFn) {
        ...
        const body = this.parseArrowBody();
        const end = this.endLoc();
        return createArrowFn(params, body, isAsync, this.loc(start, end));
      }
  ...
  parseNullish() {
    ...
    while (this.at('??')) {
      ...
      const end = this.endLoc();
      l = createBinaryExpr(op, l, r, this.loc(start, end));
    }
    return l;
  }
  ...
  parseTernary() {
    ...
    if (this.at('?')) {
      ...
      const end = this.endLoc();
      return createTernaryExpr(l, consequent, alternate, this.loc(start, end));
    }
    return l;
  }
  ...
  parseOr() {
    ...
    while (this.at('||')) {
      ...
      const end = this.endLoc();
      l = createBinaryExpr('||', l, r, this.loc(start, end));
    }
    return l;
  }
  ...
  parseUnary() {
    ...
    if (this.at('!') || this.at('-')) {
      ...
      const end = this.endLoc();
      return createUnaryExpr(op, arg, this.loc(start, end));
    }
    if (this.at('++') || this.at('--')) {
      const op = this.eat(this.t.kind).text;
      const arg = this.parseUnary();
      const end = this.endLoc();
      return createUpdateExpr(op, arg, true, this.loc(start, end));
    }
    ...
    if (this.at('spawn') || this.at('go')) {
      ...
      const end = this.endLoc();
      return createSpawnExpr(arg, this.loc(start, end));
    }
    if (this.at('yield')) {
      this.eat('yield');
      ...
      if (this.at(';') || this.at('}') || this.at(')') || this.at(',') || this.at(']') ||
        ...
        const end = this.endLoc();
        return createYieldExpr(null, this.loc(start, end));
      }
      const arg = this.parseArrow();
      const end = this.endLoc();
      return createYieldExpr(arg, this.loc(start, end));
    }
    if (this.at('new')) {
      this.eat('new');
      ...
      const end = this.endLoc();
      let expr = createNewExpr(callee, args, this.loc(start, end));
  ...
  applyPostfixChains(expr) {
    while (true) {
      ...
      if (this.at('?.')) {
        ...
        const end = this.endLoc();
        expr = createOptionalMemberExpr(expr, prop, this.loc(start, end));
      } else if (this.at('.')) {
        this.eat('.');
        const prop = this.eatPropertyName();
        const end = this.endLoc();
        expr = createMemberExpr(expr, prop, this.loc(start, end));
      } else if (this.at('[')) {
        this.eat('[');
        const index = this.parseExpr();
        this.eat(']');
        const end = this.endLoc();
        expr = createIndexExpr(expr, index, this.loc(start, end));
      } else if (this.at('(')) {
        this.eat('(');
        const args = [];
        if (!this.at(')')) {
        ...
        this.eat(')');
        const end = this.endLoc();
        expr = createCallExpr(expr, args, this.loc(start, end));
      } else if (this.at('++') || this.at('--')) {
        const op = this.eat(this.t.kind).text;
        const end = this.endLoc();
        expr = createUpdateExpr(op, expr, false, this.loc(start, end));
      } else {
        break;
      }
    }
  ...
  parsePrim() {
    ...
    if (this.at('ident')) {
      ...
      const name = this.eat('ident').text;
      const end = this.endLoc();
      return createIdentifier(name, this.loc(start, end));
    }
    if (this.at('number')) {
      const start = this.startLoc();
      const n = Number(this.eat('number').text.replaceAll('_', ''));
      const end = this.endLoc();
      return createNumberLiteral(n, this.loc(start, end));
    }
    if (this.at('string')) {
      const start = this.startLoc();
      ...
      const end = this.endLoc();
      if (s[0] === '`') {
        return createTemplateLiteral(s, this.loc(start, end));
      }
      return createStringLiteral(s.slice(1, -1), this.loc(start, end));
    }
    if (this.at('true')) {
      const start = this.startLoc();
      this.eat('true');
      const end = this.endLoc();
      return createBooleanLiteral(true, this.loc(start, end));
    }
    if (this.at('false')) {
      const start = this.startLoc();
      this.eat('false');
      const end = this.endLoc();
      return createBooleanLiteral(false, this.loc(start, end));
    }
    if (this.at('null')) {
      const start = this.startLoc();
      this.eat('null');
      const end = this.endLoc();
      return createNullLiteral(this.loc(start, end));
    }
    if (this.at('[')) {
      const start = this.startLoc();
      this.eat('[');
      ...
      if (!this.at(']')) {
        ...
        if (this.at('...')) {
          ...
          elements.push(createSpreadElement(argument, this.loc(spreadStart, spreadEnd)));
        } else {
      ...
      this.eat(']');
      const end = this.endLoc();
      return createArrayExpr(elements, this.loc(start, end));
    }
    if (this.at('{')) {
      const start = this.startLoc();
      this.eat('{');
      ...
      if (!this.at('}')) {
        ...
        if (this.at('...')) {
          ...
          const argument = this.parseExpr();
          const spreadEnd = this.endLoc();
          properties.push(createSpreadProperty(argument, this.loc(spreadStart, spreadEnd)));
        } else if (this.at('[')) {
          // Computed property
          this.eat('[');
          ...
          if (this.at(':')) {
            ...
            const value = createIdentifier(key, this.loc(identStart, identEnd));
            properties.push({ key, value, shorthand: true });
          }
        }
      ...
      this.eat('}');
      const end = this.endLoc();
      return createObjectExpr(properties, this.loc(start, end));
    }
    if (this.at('async')) {
    ...
    if (this.at('import')) {
      ...
      if (next === '(') {
        ...
        this.eat(')');
        const end = this.endLoc();
        return createImportExpr(source, this.loc(start, end));
      } else {
        // import.meta or other property access
        const name = this.eat('import').text;
        const end = this.endLoc();
        return createIdentifier(name, this.loc(start, end));
      }
    }
    ...
    const start = this.startLoc();
    const end = this.endLoc();
    return createIdentifier('__error__', this.loc(start, end));
  }
  ...
  parseSelect() {
    ...
    while (!this.at('}')) {
      if (this.at('case')) {
        ...
        // Parse select case: can be 'recv ch', 'send ch val', or 'var = await expr'
        let op;
        let channel = null;
        let value = null;
        let awaitExpr = null;
        ...
        if (!this.at('ident')) {
          const error = ParserErrors.invalidSelectCase(this, 'expected recv, send, or variable name');
          this.reportError(error);
          this.recover();
          continue;
        }

        const firstIdent = this.t.text;
        this.eat('ident');

        // Check if this is 'var = await expr' pattern
        if (this.at('=')) {
          // Pattern: var = await expr
          varName = firstIdent;
          this.eat('=');
          if (!this.at('await')) {
            const error = ParserErrors.invalidSelectCase(this, 'expected await after = in select case');
            this.reportError(error);
            this.recover();
            continue;
          }
          this.eat('await');
          awaitExpr = this.parseExpr();
          op = 'await';
        } else if (firstIdent === 'recv' || firstIdent === 'send') {
          // Pattern: recv ch or send ch val
          op = firstIdent;
          ...
          const error = ParserErrors.invalidSelectCase(this, `expected recv or send, got '${firstIdent}'`);
          this.reportError(error);
          this.recover();
          continue;
        }
        ...
        cases.push({ op, channel, value, body, awaitExpr, varName });
    ...
    const end = this.endLoc();
    return createSelectExpr(cases, defaultCase, this.loc(start, end));
  }
}
