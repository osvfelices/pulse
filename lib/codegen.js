...
export function emitProgram(p, imports = []){
  ...
  // Check if we need runtime imports
  const needsScheduler = checkNeedsScheduler(p);
  const needsChannels = checkNeedsChannels(p);
  const hasContracts = p.body.some(s => s.kind === 'ContractDecl');

  // Import runtime if needed
  if(needsScheduler || needsChannels) {
    const runtimeImports = [];
    if(needsScheduler) runtimeImports.push('spawn', 'yieldTask as yield', 'sleep');
    if(needsChannels) runtimeImports.push('channel', 'select', 'selectCase');
    out.push(`import { ${runtimeImports.join(', ')} } from './lib/runtime/index.js';`);
  }
...
// Check if program uses spawn/yield features
function checkNeedsScheduler(node) {
  if (!node) return false;
  if (node.kind === 'SpawnExpr' || node.kind === 'YieldExpr') return true;
  if (Array.isArray(node)) {
    return node.some(checkNeedsScheduler);
  }
  if (typeof node === 'object') {
    return Object.values(node).some(checkNeedsScheduler);
  }
  return false;
}

// Check if program uses channels
function checkNeedsChannels(node) {
  if (!node) return false;
  // This is a heuristic - look for calls to 'channel' or 'select'
  if (node.kind === 'Identifier' && (node.name === 'channel' || node.name === 'select')) return true;
  // Also check for SelectExpr AST nodes
  if (node.kind === 'SelectExpr') return true;
  if (Array.isArray(node)) {
    return node.some(checkNeedsChannels);
  }
  if (typeof node === 'object') {
    return Object.values(node).some(checkNeedsChannels);
  }
  return false;
}
...
function emitSelectExpr(e){
  // Emit select expression: select { case recv ch1, case send ch2 val, ... }
  // Compiles to: select([selectCase({channel: ch1, op: 'recv'}), ...])
  const cases = e.cases.map(c => {
    const channel = emitExpr(c.channel);
    const op = c.op;
    if(op === 'send'){
      const value = emitExpr(c.value);
      return `selectCase({ channel: ${channel}, op: 'send', value: ${value} })`;
    } else {
      return `selectCase({ channel: ${channel}, op: 'recv' })`;
    }
  });
