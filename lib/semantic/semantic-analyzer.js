...
export class SemanticAnalyzer {
  ...
  /**
   * Convert parser type annotation to runtime type descriptor
   * @param {Object} typeAnnotation - Parser type annotation format { base, elementType? }
   * @returns {Object|null} Runtime type descriptor or null if invalid
   */
  convertTypeAnnotation(typeAnnotation) {
    if (!typeAnnotation || !typeAnnotation.base) {
      return null;
    }

    const base = typeAnnotation.base;

    // Map parser type names to TypeKind
    const primitiveMap = {
      'int': TypeKind.Int,
    ...
    // Primitive types
    if (primitiveMap[base]) {
      return primitiveType(primitiveMap[base]);
    }

    // Generic types
    if (base === 'Channel') {
      if (typeAnnotation.elementType) {
        const elementDesc = this.convertTypeAnnotation(typeAnnotation.elementType);
        return elementDesc ? channelType(elementDesc) : channelType();
      }
      return channelType();
    }
    ...
    if (base === 'Array') {
      if (typeAnnotation.elementType) {
        const elementDesc = this.convertTypeAnnotation(typeAnnotation.elementType);
        return elementDesc ? arrayType(elementDesc) : arrayType();
      }
      return arrayType();
    }
    ...
    if (base === 'Task') {
      if (typeAnnotation.elementType) {
        const resultDesc = this.convertTypeAnnotation(typeAnnotation.elementType);
        return resultDesc ? taskType(resultDesc) : taskType();
      }
      return taskType();
    }

    // Object type (no generic support yet)
    if (base === 'Object' || base === 'object') {
      return objectType();
    }

    // Function type (simplified)
    if (base === 'Function' || base === 'function') {
      return functionType();
    }

    // Unknown type - return null (don't create invalid descriptors)
    return null;
  }
  ...
  visitFnDecl(node) {
    ...
    if (node.name) {
      ...
      if (node.returnType) {
        const returnTypeDesc = this.convertTypeAnnotation(node.returnType);
        if (returnTypeDesc) {
          // Store return type in symbol's type descriptor as a function type
          const paramTypes = [];
          for (const param of node.params) {
            if (param.typeAnnotation) {
              const paramTypeDesc = this.convertTypeAnnotation(param.typeAnnotation);
              paramTypes.push(paramTypeDesc || null);
            } else {
              paramTypes.push(null);
            }
          }
          symbol.typeDescriptor = functionType(paramTypes.length > 0 ? paramTypes : null, returnTypeDesc);
        }
      }
    ...
    // Store return type in function scope metadata for later use
    if (node.returnType) {
      const returnTypeDesc = this.convertTypeAnnotation(node.returnType);
      if (returnTypeDesc) {
        this.currentScope.returnType = returnTypeDesc;
      }
    }
    ...
    for (const param of node.params) {
      if (param.name) {
        ...
        // Extract parameter type annotation if present
        if (param.typeAnnotation) {
          const paramTypeDesc = this.convertTypeAnnotation(param.typeAnnotation);
          if (paramTypeDesc) {
            paramSymbol.typeDescriptor = paramTypeDesc;
          }
        }
  ...
  visitVarDecl(node) {
    ...
    if (name) {
      ...
      if (node.typeAnnotation) {
        const typeDesc = this.convertTypeAnnotation(node.typeAnnotation);
        if (typeDesc) {
          symbol.typeDescriptor = typeDesc;
        }
      }
    ...
    for (const param of node.params) {
      if (typeof param === 'string') {
        ...
        if (param.typeAnnotation) {
          const paramTypeDesc = this.convertTypeAnnotation(param.typeAnnotation);
          if (paramTypeDesc) {
            symbol.typeDescriptor = paramTypeDesc;
          }
        }
