...
function collectRegisterOperands(instruction) {
  ...
  // Collect from iterator instructions
  if (instruction.iterable && isRegister(instruction.iterable)) {
    registers.push(instruction.iterable);
  }
  if (instruction.iterator && isRegister(instruction.iterator)) {
    registers.push(instruction.iterator);
  }
  if (instruction.iteratorResult && isRegister(instruction.iteratorResult)) {
    registers.push(instruction.iteratorResult);
  }

  // Collect from async instructions
  if (instruction.promise && isRegister(instruction.promise)) {
    registers.push(instruction.promise);
  }

  // Collect from channel instructions
  if (instruction.channel && isRegister(instruction.channel)) {
    registers.push(instruction.channel);
  }

  // Collect from Select cases
  if (instruction.cases && Array.isArray(instruction.cases)) {
    for (const selectCase of instruction.cases) {
      if (selectCase.channel && isRegister(selectCase.channel)) {
        registers.push(selectCase.channel);
      }
      if (selectCase.value && isRegister(selectCase.value)) {
        registers.push(selectCase.value);
      }
      // Switch cases
      if (selectCase.test && isRegister(selectCase.test)) {
        registers.push(selectCase.test);
      }
    }
  }

  // Collect from exception handling instructions
  if (instruction.exceptionReg && isRegister(instruction.exceptionReg)) {
    registers.push(instruction.exceptionReg);
  }

  // Collect from switch discriminant
  if (instruction.discriminant && isRegister(instruction.discriminant)) {
    registers.push(instruction.discriminant);
  }

  // Collect from MethodCall
  if (instruction.property && isRegister(instruction.property)) {
    registers.push(instruction.property);
  }
  ...
  for (const func of irModule.functions) {
    ...
    for (const block of func.blocks) {
      ...
      for (let i = 0; i < block.instructions.length; i++) {
        ...
        // Validate async/await usage: Await instruction only allowed in async functions
        if (instr.kind === InstructionKinds.Await) {
          if (!func.async) {
            errors.push(new IRValidationError(
              'Await instruction found in non-async function',
              functionName,
              blockLabel,
              i
            ));
        ...
        // Validate Select with await cases: await cases only in async functions
        if (instr.kind === InstructionKinds.Select) {
          if (instr.cases && Array.isArray(instr.cases)) {
            for (let caseIdx = 0; caseIdx < instr.cases.length; caseIdx++) {
              const selectCase = instr.cases[caseIdx];
              if (selectCase.isAwaitCase && !func.async) {
                errors.push(new IRValidationError(
                  `Select case ${caseIdx} with await requires async function context`,
                  functionName,
                  blockLabel,
                  i
                ));
