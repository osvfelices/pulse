      ...
      for (let i = 0; i < block.instructions.length; i++) {
        ...
        if (isTerminator(instr)) {
          ...
          if (instr.kind === InstructionKinds.CondJump) {
            if (!blockLabels.has(instr.thenBlock)) {
              errors.push(new IRValidationError(
                `CondJump to undefined then block: ${instr.thenBlock}`,
                functionName,
                blockLabel,
                i
              ));
            }
            if (!blockLabels.has(instr.elseBlock)) {
              errors.push(new IRValidationError(
                `CondJump to undefined else block: ${instr.elseBlock}`,
                functionName,
                blockLabel,
                i
              ));
