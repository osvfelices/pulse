...
test('Lowers select with recv cases', () => {
  ...
  // Find Await instruction wrapping select
...
test('Lowers await expression', () => {
  ...
  // Find Call to fetchData
  const callInstr = block.instructions.find(i => i.kind == InstructionKinds.Call);
  assert(callInstr, 'Should have Call instruction');
