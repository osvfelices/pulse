    ...
    if (pattern.kind === NodeKinds.ArrayPattern) {
      ...
      for (let i = 0; i < pattern.elements.length; i++) {
        ...
        if (elem.kind === NodeKinds.RestElement) {
          // Rest element: ...rest
          if (!elem.argument) {
            throw new Error('RestElement requires argument');
          }

          const sliceMethod = this.allocRegister();
          this.emit({
          ...
          if (elem.argument.kind === NodeKinds.Identifier) {
            const reg = this.allocRegister(elem.argument.name);
            this.symbolMap.set(elem.argument.name, reg);
            this.emit({
              kind: InstructionKinds.Assign,
              dest: reg,
              value: sliceResult,
