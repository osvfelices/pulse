...
// Test unsupported nodes throw
test('Throws on unsupported node kinds', () => {
  const src = `
    fn test() {
      if (true) {
        return 1;
  ...
  try {
    ...
    threw = true;
    assert(e.message.includes('Unsupported'), 'Error message mentions unsupported');
  }
  assert(threw, 'Should throw on if statement');
});
