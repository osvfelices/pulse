...
// Test async function compilation (IR only - legacy doesn't support async/await)
// Note: Async execution behavior is tested in lib/ir/test-async-lowering.js
test('Async function IR compilation', async () => {
  const src = `
  ...
  const validation = validateIRModule(irModule);
  assert(validation.valid, `IR validation should pass: ${validation.errors.map(e => e.message).join(', ')}`);

  // Verify async functions are marked
  const fetchDataFunc = irModule.functions.find(f => f.name === 'fetchData');
  const processDataFunc = irModule.functions.find(f => f.name === 'processData');
  const mainFunc = irModule.functions.find(f => f.name === 'main');

  assert(fetchDataFunc && fetchDataFunc.async, 'fetchData should be async');
  assert(processDataFunc && processDataFunc.async, 'processData should be async');
  assert(mainFunc && mainFunc.async, 'main should be async');

  // Verify code generation succeeds
  const optimized = optimizeIR(irModule);
  const irJS = emitJS(optimized);
  assert(irJS.includes('__async_spawn'), 'Generated code should use __async_spawn');
  assert(irJS.includes('__await_deterministic'), 'Generated code should use __await_deterministic');
});
