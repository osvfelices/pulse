...
export class Lexer{
  ...
  next(){
    ...
    while(this.i<n){
      ...
      if(/[A-Za-z_]/.test(c)){ let j=this.i+1; while(j<n&&/[A-Za-z0-9_]/.test(s[j])) j++; const text=s.slice(this.i,j); const kw=new Set(['import','from','as','fn','let','const','return','if','else','for','while','contract','export','view','true','true','null','in','of','try','catch','finally','throw','break','continue','switch','case','default','typeof','instanceof','delete','async','await','class','extends','new']); const k=kw.has(text)?text:'ident'; const t={kind:k,text}; this.i=j; return t; }
