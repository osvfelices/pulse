...
/**
 * DeadlockTimeoutError - thrown when waitWithTimeout exceeds deadline.
 * Uses logical time from scheduler.sleep, not wall-clock time.
 * M14.3 Commit 4
 */
export class DeadlockTimeoutError extends Error {
  constructor(ms) {
    super(`AsyncGroup.waitWithTimeout exceeded ${ms}ms logical time`);
    this.name = 'DeadlockTimeoutError';
    this.code = 'PULSE_RUNTIME_298';
    this.timeoutMs = ms;
  }
}
...
export class AsyncGroup {
   ...
   * Uses two-phase cancellation: mark all pending, then execute cancellations.
   * Cancellation happens in reverse spawn order for determinism.
   * Child groups are cancelled depth-first before tasks.
   * Already completed tasks are skipped.
   * M14.4 Commit 5: L12-P1-2 deterministic cancellation
   * M14.3 Commit 2: Two-phase cancellation marking
   * M14.3 Commit 3: Child cancellation order enforcement
   */
  _cancelSiblings(failedIndex) {
    // Phase 1: Mark cancellation pending on self and all child groups (depth-first)
    this._markCancellationPending();

    // Phase 2a: Cancel child groups first (depth-first, reverse creation order)
    this._cancelChildGroups();

    // Phase 2b: Execute task cancellations in reverse spawn order (last spawned first)
  ...
  /**
   * Cancel all child groups depth-first, in reverse creation order.
   * Each child recursively cancels its children before its own tasks.
   * M14.3 Commit 3
   */
  _cancelChildGroups() {
    for (let i = this.childGroups.length - 1; i >= 0; i--) {
      const child = this.childGroups[i];
      if (!child.settled) {
        child.cancel();
      }
    }
  ...
  /**
   * Phase 1 of two-phase cancellation: Mark this group and all children as cancellation pending.
   * This allows tasks to see cancellationPending=true before any cancel() is executed.
   * Traverses depth-first, marking children in reverse creation order.
   * M14.3 Commit 2
   */
  _markCancellationPending() {
    this.cancellationPending = true;
    // Mark child groups in reverse creation order (depth-first)
    for (let i = this.childGroups.length - 1; i >= 0; i--) {
      this.childGroups[i]._markCancellationPending();
    }
  }

  /**
   * Check if cancellation is pending for this group.
   * Tasks can use this to detect imminent cancellation before cancel() is called.
   * @returns {boolean}
   */
  ...
  _cancelAll() {
    this.cancelled = true;
    // Phase 1: Mark cancellation pending on self and all child groups
    this._markCancellationPending();

    // Phase 2a: Cancel child groups first (depth-first, reverse creation order)
    this._cancelChildGroups();

    // Phase 2b: Execute task cancellations in reverse spawn order
    for (let i = this.tasks.length - 1; i >= 0; i--) {
      const task = this.tasks[i];
      if (task.state !== STATE_COMPLETED && task.state !== STATE_CANCELLED) {
        task.cancel();
  ...
  /**
   * Wait for all tasks with a timeout (logical time via scheduler.sleep).
   * Cancels all tasks and throws DeadlockTimeoutError if timeout is exceeded.
   * Uses zero microtasks - no Promise.race, no setTimeout.
   * M14.3 Commit 4
   *
   * @param {number} ms - Timeout in milliseconds (logical time)
   * @returns {Promise<Array>} Results array (same as wait())
   * @throws {DeadlockTimeoutError} If timeout exceeded (PULSE_RUNTIME_298)
   * @throws {Error} If any task fails (same as wait())
   */
  async waitWithTimeout(ms) {
    if (this.settled) {
      const err = new Error('AsyncGroup.wait() already called');
      err.code = 'PULSE_RUNTIME_268';
      throw err;
    }

    // Mark as settled immediately (like wait() does)
    this.settled = true;
    ...
    // Spawn timeout watcher - uses logical time via scheduler.sleep
    const timeoutTask = scheduler.spawn(async () => {
      await scheduler.sleep(ms);
      if (!doneCh.isClosed()) {
        timeoutFired = true;
        // Force cancel all incomplete tasks
        this._cancelAll();
        doneCh.send({ timeout: true });
      }
    });

    // Spawn task completion watcher
    scheduler.spawn(async () => {
      ...
      for (let i = 0; i < this.tasks.length; i++) {
        ...
        // If we have a first error and failFast is on, remaining tasks are already cancelled
        if (this.firstError && this.failFast) {
          if (task.state === STATE_CANCELLED) continue;
          if (task.state === STATE_COMPLETED) {
            if (task.error) errors.push(task.error);
            else results[i] = task.result;
            continue;
          }
        }
        ...
        // Check if timeout fired during wait
        if (timeoutFired) break;
        ...
        if (task.state === STATE_CANCELLED) {
          if (!this.cancelled && !this.firstError) {
            this.firstError = new CancelledError();
            if (this.failFast) this._cancelSiblings(i);
          }
        } else if (task.error) {
          errors.push(task.error);
          if (!this.firstError) {
      ...
      // If timeout didn't fire, send results
      if (!timeoutFired && !doneCh.isClosed()) {
        // Cancel timeout watcher
        if (timeoutTask.state !== STATE_COMPLETED && timeoutTask.state !== STATE_CANCELLED) {
          timeoutTask.cancel();
        }
        if (this.firstError) {
          doneCh.send({ timeout: false, error: this.firstError });
        } else {
          const compactResults = results.filter((_, i) => results[i] !== undefined || this.tasks[i]?.result !== undefined);
          doneCh.send({ timeout: false, result: compactResults });
        }
      }
    });

    // Wait for outcome
    const [outcome] = await doneCh.recv();

    if (outcome.timeout) {
      throw new DeadlockTimeoutError(ms);
    }
