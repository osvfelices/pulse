...
import { getSupervisorRegistry } from './supervisor.js';
...
export class Inspector {
  ...
  getSupervisors() {
    ...
    const registry = getSupervisorRegistry();
    const supervisors = [];

    for (const sup of registry.values()) {
      supervisors.push(sup.getSnapshot());
    }
  ...
  /**
   * Get supervisor tree (M14.2)
   * Returns hierarchical view of supervisors
   */
  getSupervisorTree() {
    if (!this.enabled) {
      return {
    ...
    for (const sup of registry.values()) {
      ...
      if (!snapshot.parentSupervisorId) {
        roots.push(snapshot);
      }
    }
    ...
    const buildTree = (node) => {
      const children = supervisors.filter(s => s.parentSupervisorId === node.__id);
      return {
        ...node,
        childSupervisors: children.map(buildTree)
      };
    };
  ...
  getSupervisorById(supervisorId) {
    ...
    const registry = getSupervisorRegistry();
    const supervisor = registry.get(supervisorId);

    if (!supervisor) {
      return {
        ok: false,
        code: 'SUPERVISOR_NOT_FOUND',
        error: `Supervisor ${supervisorId} not found`
      };
    }

    return {
      ok: true,
      supervisor: supervisor.getSnapshot()
    };
  }
