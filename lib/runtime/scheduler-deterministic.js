/**
 * Pulse Deterministic Scheduler V2
 *
 * Purely logical scheduler with no dependence on setImmediate, setTimeout,
 * or Promise.race. Achieves determinism through controlled promise resolution.
 *
 * Key principles:
 * - step() is synchronous, processes scheduler logic
 * - flush() resolves pending promises in deterministic order
 * - Only uses await Promise.resolve() for one microtask tick
 * - Cross-platform: works in Node, Deno, Bun, browser
 */

const PRIORITY_HIGH = 0;
const PRIORITY_NORMAL = 1;
const PRIORITY_LOW = 2;

const STATE_PENDING = 'pending';
const STATE_RUNNING = 'running';
const STATE_SLEEPING = 'sleeping';
const STATE_COMPLETED = 'completed';
const STATE_CANCELLED = 'cancelled';

class CancelledError extends Error {
  constructor() {
    super('Task was cancelled');
    this.name = 'CancelledError';
  }
}

let taskIdCounter = 0;

class Task {
  constructor(fn, priority, scheduler) {
    this.id = ++taskIdCounter;
    this.fn = fn;
    this.priority = priority;
    this.state = STATE_PENDING;
    this.scheduler = scheduler;
    this.result = null;
    this.error = null;
    this.wakeTime = null;
    this.continuation = null;
    this.promise = null;
    this.createdAt = scheduler.logicalTime;
    this.started = false; // Track if task has ever run
  }

  cancel() {
    if (this.state === STATE_COMPLETED || this.state === STATE_CANCELLED) {
      return;
    }

    this.state = STATE_CANCELLED;
    this.scheduler.removeTask(this);

    if (this.continuation) {
      this.continuation.reject(new CancelledError());
      this.continuation = null;
    }
  }
}

class PriorityQueues {
  constructor() {
    // Separate queues for new tasks vs resuming tasks
    // New tasks are processed first to ensure all tasks start before any resume
    this.newQueues = {
      [PRIORITY_HIGH]: [],
      [PRIORITY_NORMAL]: [],
      [PRIORITY_LOW]: []
    };
    this.resumeQueues = {
      [PRIORITY_HIGH]: [],
      [PRIORITY_NORMAL]: [],
      [PRIORITY_LOW]: []
    };
  }

  enqueue(task) {
    const targetQueues = task.started ? this.resumeQueues : this.newQueues;
    targetQueues[task.priority].push(task);
  }

  dequeue() {
    // First try new tasks (never started) by priority
    for (const priority of [PRIORITY_HIGH, PRIORITY_NORMAL, PRIORITY_LOW]) {
      if (this.newQueues[priority].length > 0) {
        return this.newQueues[priority].shift();
      }
    }

    // Then try resuming tasks by priority
    for (const priority of [PRIORITY_HIGH, PRIORITY_NORMAL, PRIORITY_LOW]) {
      if (this.resumeQueues[priority].length > 0) {
        return this.resumeQueues[priority].shift();
      }
    }

    return null;
  }

  remove(task) {
    const newQueue = this.newQueues[task.priority];
    const resumeQueue = this.resumeQueues[task.priority];

    let index = newQueue.indexOf(task);
    if (index !== -1) {
      newQueue.splice(index, 1);
      return;
    }

    index = resumeQueue.indexOf(task);
    if (index !== -1) {
      resumeQueue.splice(index, 1);
    }
  }

  isEmpty() {
    for (const priority of [PRIORITY_HIGH, PRIORITY_NORMAL, PRIORITY_LOW]) {
      if (this.newQueues[priority].length > 0 || this.resumeQueues[priority].length > 0) {
        return false;
      }
    }
    return true;
  }

  size() {
    let total = 0;
    for (const priority of [PRIORITY_HIGH, PRIORITY_NORMAL, PRIORITY_LOW]) {
      total += this.newQueues[priority].length + this.resumeQueues[priority].length;
    }
    return total;
  }
}

class DeterministicScheduler {
  constructor() {
    this.readyQueue = new PriorityQueues();
    this.sleepQueue = []; // sorted by wakeTime ascending
    this.logicalTime = 0;
    this.currentTask = null;
    this.currentTaskId = null; // ID of currently running task
    this.allTasks = new Map();
    this.allChannels = new Map(); // Registry for all channels
    this.resolutionQueue = []; // promises to resolve in order (used by flush)
    this.running = false;
    this.pendingSpawns = 0; // tasks spawned but not yet started

    // Per-scheduler PRNG state (P0-NEW-1 fix: deterministic across schedulers)
    // Each scheduler has its own independent PRNG to ensure determinism
    // even when multiple schedulers exist (e.g., in tests or nested runs)
    this.prngState = null;

    // M14.4: Promise registry for PulsePromise tracking
    // Maps promise ID to PulsePromise instance for inspector/snapshot support
    this.pendingPromises = new Map();
    this.nextPromiseId = 1;

    // M14.4: AsyncGroup ID counter for deterministic group identification
    this.nextGroupId = 1;

    // Instrumentation counters (only in NODE_ENV=test or when PULSE_DEBUG=1)
    this.stats = {
      tasksCreated: 0,
      tasksCompleted: 0,
      tasksBlocked: 0,
      sleepsScheduled: 0,
      sleepsWoken: 0,
      stepsExecuted: 0,
      idleCycles: 0,
      maxReadyQueue: 0,
      maxSleepQueue: 0
    };
    this.enableStats = process.env.NODE_ENV === 'test' || process.env.PULSE_DEBUG === '1';
  }

  /**
   * Get current logical time
   */
  getLogicalTime() {
    return this.logicalTime;
  }

  /**
   * Get internal statistics for debugging and testing
   * Only available when NODE_ENV=test or PULSE_DEBUG=1
   */
  getStats() {
    if (!this.enableStats) {
      return null;
    }
    return {
      ...this.stats,
      currentReadyQueue: this.readyQueue.size(),
      currentSleepQueue: this.sleepQueue.length,
      logicalTime: this.logicalTime,
      currentTask: this.currentTask ? this.currentTask.id : null
    };
  }

  /**
   * Reset statistics counters
   */
  resetStats() {
    if (!this.enableStats) return;
    this.stats = {
      tasksCreated: 0,
      tasksCompleted: 0,
      tasksBlocked: 0,
      sleepsScheduled: 0,
      sleepsWoken: 0,
      stepsExecuted: 0,
      idleCycles: 0,
      maxReadyQueue: 0,
      maxSleepQueue: 0
    };
  }

  spawn(fn, options = {}) {
    const priority = options.priority !== undefined ? options.priority : PRIORITY_NORMAL;
    const task = new Task(fn, priority, this);

    this.allTasks.set(task.id, task);
    this.readyQueue.enqueue(task);
    this.pendingSpawns++; // track unstarted spawn

    return task;
  }

  yield() {
    if (!this.currentTask) {
      return Promise.resolve();
    }

    return new Promise((resolve, reject) => {
      this.currentTask.state = STATE_PENDING;
      this.currentTask.continuation = { resolve, reject };
      this.readyQueue.enqueue(this.currentTask);
    });
  }

  sleep(ms) {
    if (!this.currentTask) {
      throw new Error('sleep() can only be called from within a scheduled task');
    }

    const wakeTime = this.logicalTime + ms;

    return new Promise((resolve, reject) => {
      this.currentTask.state = STATE_SLEEPING;
      this.currentTask.wakeTime = wakeTime;
      this.currentTask.continuation = { resolve, reject };

      // Insert into sleep queue in sorted order
      const insertIndex = this.sleepQueue.findIndex(t => t.wakeTime > wakeTime);
      if (insertIndex === -1) {
        this.sleepQueue.push(this.currentTask);
      } else {
        this.sleepQueue.splice(insertIndex, 0, this.currentTask);
      }
    });
  }

  removeTask(task) {
    // Remove from ready queue
    this.readyQueue.remove(task);

    // Remove from sleep queue
    const sleepIndex = this.sleepQueue.indexOf(task);
    if (sleepIndex !== -1) {
      this.sleepQueue.splice(sleepIndex, 1);
    }

    this.allTasks.delete(task.id);
  }

  processWakeups() {
    // Don't wake tasks if there are pending spawns that haven't started yet
    // This ensures spawn() + sleep() waits for spawned tasks to begin
    if (this.pendingSpawns > 0) {
      return;
    }

    while (this.sleepQueue.length > 0 &&
           this.sleepQueue[0].wakeTime <= this.logicalTime) {
      const task = this.sleepQueue.shift();

      if (task.state === STATE_CANCELLED) {
        continue;
      }

      task.state = STATE_PENDING;
      task.wakeTime = null;
      this.readyQueue.enqueue(task);
      // Don't clear continuation here - step() will handle it
    }
  }

  startTask(task) {
    if (task.state === STATE_CANCELLED) {
      this.allTasks.delete(task.id);
      return;
    }

    task.started = true; // Mark as started
    task.state = STATE_RUNNING;
    this.currentTask = task;
    this.currentTaskId = task.id;
    this.pendingSpawns--; // task has now started

    const taskResult = task.fn();

    // Handle PulsePromise specially to avoid double-spawning via .then()
    if (taskResult && taskResult.__pulse_async) {
      // PulsePromise: receive from channel directly instead of calling .then()
      task.promise = (async () => {
        try {
          const [result] = await taskResult.__result_ch.recv();
          if (task.state !== STATE_CANCELLED) {
            task.result = result.ok ? result.value : null;
            if (!result.ok) {
              task.error = result.error;
            }
            task.state = STATE_COMPLETED;
          }
          this.allTasks.delete(task.id);
        } catch (error) {
          if (task.state !== STATE_CANCELLED) {
            task.error = error;
            task.state = STATE_COMPLETED;
          }
          this.allTasks.delete(task.id);
        }
      })();
    } else {
      // Regular Promise: use .then()
      task.promise = taskResult.then(
        result => {
          if (task.state !== STATE_CANCELLED) {
            task.result = result;
            task.state = STATE_COMPLETED;
          }
          this.allTasks.delete(task.id);
        },
        error => {
          if (task.state !== STATE_CANCELLED) {
            task.error = error;
            task.state = STATE_COMPLETED;
          }
          this.allTasks.delete(task.id);
        }
      );
    }

    this.currentTask = null;
    this.currentTaskId = null;
  }

  step() {
    // Process wakeups (moves sleeping tasks to ready queue)
    this.processWakeups();

    // Get next ready task
    const task = this.readyQueue.dequeue();

    if (!task) {
      // No ready tasks
      // Check if we have sleeping tasks that need to wake up
      if (this.sleepQueue.length > 0) {
        // Advance time to next wakeup FIRST
        // This is critical: even if tasks are running/blocked on channels,
        // we must wake up sleepers that might unblock them
        this.logicalTime = this.sleepQueue[0].wakeTime;
        return 'idle';
      }

      // Check if any tasks are still running (in progress)
      const runningTasks = Array.from(this.allTasks.values()).filter(
        t => t.state === STATE_RUNNING || t.state === STATE_PENDING
      );
      if (runningTasks.length > 0) {
        return 'wait'; // Need to wait for async tasks to progress
      }

      // Check if any tasks still exist
      if (this.allTasks.size === 0) {
        return 'done';
      }

      return 'idle';
    }

    // Resume or start task
    if (task.continuation) {
      // Task is suspended, resume it
      const cont = task.continuation;
      task.continuation = null;
      task.state = STATE_RUNNING;
      // Queue the resolution to be executed during flush()
      // This ensures deterministic ordering of promise resolutions
      this.resolutionQueue.push(() => {
        // Set currentTask when the continuation executes
        // It stays set through flush() so resumed tasks can call scheduler methods
        this.currentTask = task;
        this.currentTaskId = task.id;
        cont.resolve();
      });
    } else {
      // Task has never run, start it
      this.startTask(task);
    }

    // Advance logical time by 1 tick per task execution
    // BUT: don't advance time if there are pending spawns
    // This ensures spawned tasks start before their parent's sleep wakes
    if (this.pendingSpawns === 0) {
      this.logicalTime++;
    }

    return 'stepped';
  }

  /**
   * Flush the resolution queue.
   *
   * Executes all queued promise resolutions in FIFO order, then allows
   * two microtask ticks for async functions to resume and completion
   * handlers to run. This is the central mechanism for deterministic
   * promise resolution in the Pulse scheduler.
   *
   * @returns {Promise<void>}
   */
  async flush() {
    // Resolve all pending promises in order
    const toResolve = this.resolutionQueue.slice();
    this.resolutionQueue.length = 0;

    for (const resolveFn of toResolve) {
      resolveFn();
    }

    // TWO microtask ticks to allow promises to settle
    // First tick: async functions resume
    // Second tick: completion handlers run
    await Promise.resolve();
    await Promise.resolve();

    // Clear currentTask after all resumed tasks have completed
    this.currentTask = null;
    this.currentTaskId = null;
  }

  async run() {
    if (this.running) {
      throw new Error('Scheduler is already running');
    }

    this.running = true;

    while (this.running) {
      const status = this.step();
      await this.flush();

      if (status === 'done') {
        break;
      }

      if (status === 'wait') {
        // Tasks are in progress, just wait for them
        continue;
      }

      if (status === 'idle' && this.sleepQueue.length === 0) {
        // Nothing left to do
        break;
      }

      // Continue stepping
    }

    this.running = false;
  }

  stop() {
    this.running = false;
  }

  /**
   * Drain scheduler: run until all tasks complete
   *
   * Executes the scheduler loop until all spawned tasks finish.
   * Used for module initialization to ensure async main functions
   * complete before program exit.
   *
   * @returns {Promise<void>} Resolves when all tasks are complete
   */
  async drain() {
    return this.run();
  }

  // Utility methods for inspection
  getTaskCount() {
    return this.allTasks.size;
  }

  getReadyCount() {
    return this.readyQueue.size();
  }

  getSleepingCount() {
    return this.sleepQueue.length;
  }

  getState() {
    return {
      logicalTime: this.logicalTime,
      readyCount: this.getReadyCount(),
      sleepingCount: this.getSleepingCount(),
      totalTasks: this.getTaskCount()
    };
  }

  // Accessor methods for snapshot engine
  getAllTasks() {
    return Array.from(this.allTasks.values());
  }

  getTotalTaskCount() {
    return this.allTasks.size;
  }

  getAllChannels() {
    return Array.from(this.allChannels.values());
  }

  registerChannel(channel) {
    this.allChannels.set(channel.id, channel);
  }

  unregisterChannel(channelId) {
    this.allChannels.delete(channelId);
  }

  // ============================================================================
  // PRNG Methods (P0-NEW-1: scheduler-local PRNG state)
  // ============================================================================

  /**
   * Seed this scheduler's PRNG.
   * @param {number} seed - Integer seed value
   */
  seedPRNG(seed) {
    if (typeof seed !== 'number' || !Number.isInteger(seed)) {
      throw new Error('seedPRNG requires an integer seed');
    }
    this.prngState = seed >>> 0; // Convert to unsigned 32-bit
  }

  /**
   * Get next random value from this scheduler's PRNG (Mulberry32).
   * @returns {number} Random number in [0, 1)
   * @throws {Error} If PRNG not seeded
   */
  nextRandom() {
    if (this.prngState === null) {
      throw new Error(
        'PRNG not seeded. Call seedRandom(seed) from within a Pulse task before using random functions.'
      );
    }
    let t = this.prngState += 0x6D2B79F5;
    t = Math.imul(t ^ t >>> 15, t | 1);
    t ^= t + Math.imul(t ^ t >>> 7, t | 61);
    this.prngState = t >>> 0;
    return ((t ^ t >>> 14) >>> 0) / 4294967296;
  }

  /**
   * Check if PRNG is seeded.
   * @returns {boolean}
   */
  isPRNGSeeded() {
    return this.prngState !== null;
  }

  // ============================================================================
  // M14.4: Promise Registry Methods
  // ============================================================================

  /**
   * Register a PulsePromise with the scheduler for tracking.
   *
   * Assigns a monotonic ID to the promise and stores it in the pending
   * promises registry. This enables inspector and snapshot support for
   * in-flight promises.
   *
   * @param {Object} promise - The PulsePromise instance to register
   * @returns {number} The assigned promise ID (monotonically increasing)
   */
  registerPromise(promise) {
    const id = this.nextPromiseId++;
    this.pendingPromises.set(id, promise);
    return id;
  }

  /**
   * Unregister a PulsePromise from the scheduler.
   *
   * Called when a promise settles (fulfilled, rejected, or cancelled).
   * Idempotent: no error if the ID is not present.
   *
   * @param {number} promiseId - The ID of the promise to unregister
   */
  unregisterPromise(promiseId) {
    this.pendingPromises.delete(promiseId);
  }

  /**
   * Get all currently pending PulsePromises.
   *
   * Read-only accessor for inspector and debugging. Returns an array
   * of all promise instances that have been registered but not yet settled.
   *
   * @returns {Array} Array of pending PulsePromise instances
   */
  getPendingPromises() {
    return Array.from(this.pendingPromises.values());
  }

  /**
   * Get the current length of the resolution queue.
   *
   * Read-only accessor for inspector and debugging. Returns the number
   * of promise resolutions queued for the next flush() call.
   *
   * @returns {number} Number of pending resolutions
   */
  getResolutionQueueLength() {
    return this.resolutionQueue.length;
  }

  /**
   * Get the number of pending promises.
   *
   * @returns {number} Count of registered but unsettled promises
   */
  getPendingPromiseCount() {
    return this.pendingPromises.size;
  }

  /**
   * Get a snapshot of the scheduler's promise-related state.
   *
   * Used by the snapshot engine to capture scheduler state for
   * debugging and time-travel. Does not include full promise snapshots
   * (those are captured separately by PulsePromise.getSnapshot).
   *
   * @returns {Object} Promise registry metadata
   */
  getPromiseRegistrySnapshot() {
    return {
      nextPromiseId: this.nextPromiseId,
      nextGroupId: this.nextGroupId,
      pendingPromiseCount: this.pendingPromises.size,
      resolutionQueueLength: this.resolutionQueue.length
    };
  }

  /**
   * Restore promise registry state from a snapshot.
   *
   * Used by the snapshot engine to restore scheduler state.
   * Note: This only restores ID counters; actual promises are
   * restored separately.
   *
   * @param {Object} snapshot - Snapshot from getPromiseRegistrySnapshot
   */
  restorePromiseRegistrySnapshot(snapshot) {
    this.nextPromiseId = snapshot.nextPromiseId;
    this.nextGroupId = snapshot.nextGroupId;
    // Note: pendingPromises map is NOT restored here
    // Promises are restored individually and re-register themselves
  }
}

// Global scheduler instance for convenience
let globalScheduler = null;

function getScheduler() {
  if (!globalScheduler) {
    globalScheduler = new DeterministicScheduler();
  }
  return globalScheduler;
}

function resetScheduler() {
  globalScheduler = null;
}

function spawn(fn, options) {
  return getScheduler().spawn(fn, options);
}

function yieldTask() {
  return getScheduler().yield();
}

function sleep(ms) {
  return getScheduler().sleep(ms);
}

export {
  DeterministicScheduler,
  Task,
  spawn,
  yieldTask,
  sleep,
  getScheduler,
  resetScheduler,
  PRIORITY_HIGH,
  PRIORITY_NORMAL,
  PRIORITY_LOW,
  STATE_PENDING as TASK_PENDING,
  STATE_RUNNING as TASK_RUNNING,
  STATE_SLEEPING as TASK_BLOCKED,
  STATE_COMPLETED as TASK_COMPLETED,
  STATE_CANCELLED as TASK_CANCELLED,
  CancelledError
};
