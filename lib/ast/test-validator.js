...
test('validateAST rejects FnDecl without name', () => {
  const ast = {
  ...
  const result = validateAST(ast);
  assert(result.valid === false, 'FnDecl without name should be invalid');
  assert(result.errors.some(e => e.message.includes('name')), 'Should report missing name');
});
