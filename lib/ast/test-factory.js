...
test('createImportDecl with side-effect import', () => {
  const node = createImportDecl('module-name', { sideEffect: true }, null);
  assert(node.kind === NodeKinds.ImportDecl, 'kind is ImportDecl');
  assert(node.source === 'module-name', 'source is correct');
  assert(node.sideEffect === true, 'sideEffect is true');
});

test('createImportDecl with specifiers', () => {
  const node = createImportDecl('module-name', {
    default: 'foo',
    named: [{ imported: 'bar', local: 'baz' }],
    sideEffect: false
  }, null);
  assert(node.sideEffect === false, 'sideEffect is false');
  assert(node.default === 'foo', 'default import is correct');
});
...
test('createFnDecl accepts null name for anonymous functions', () => {
  const body = createBlock([], null);
  const node = createFnDecl(null, [], body, false, null);
  assert(node.name === null, 'name is null for anonymous functions');
  assert(node.kind === NodeKinds.FnDecl, 'kind is FnDecl');
});
