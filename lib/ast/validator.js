...
/**
 * Require that a value is an array (for internal validation)
 * @param {any} value
 * @param {string} name
...
function validateNode(node, ctx) {
  ...
  switch (kind) {
    ...
    case NodeKinds.ImportDecl:
      ...
      requireField(node, 'sideEffect', ctx);
      if ('named' in node && node.named) {
        requireArrayField(node, 'named', ctx);
      }
      break;
    ...
    case NodeKinds.FnDecl:
      // name can be null for anonymous function expressions
      requireField(node, 'name', ctx);
      if (node.name !== null && typeof node.name !== 'string') {
        ctx.addError('FnDecl.name must be a string or null', node);
      }
    ...
    case NodeKinds.SelectExpr:
      requireArrayField(node, 'cases', ctx);
      // defaultCase is a plain object with { kind: 'default', body: [...] }, not an AST node
      if (node.defaultCase && node.defaultCase.body) {
        requireArray(node.defaultCase.body, 'SelectExpr.defaultCase.body');
        node.defaultCase.body.forEach(stmt => validateNode(stmt, ctx));
      }
      // Validate case bodies
      if (node.cases) {
        node.cases.forEach(c => {
          if (c.body) {
            requireArray(c.body, 'SelectExpr.case.body');
            c.body.forEach(stmt => validateNode(stmt, ctx));
          }
        });
      }
      break;
