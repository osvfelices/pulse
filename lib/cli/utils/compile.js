...
/**
 * Supported Pulse source file extensions
 * .pls is primary (Pulse 3.1+), .pulse is legacy (Pulse 3.0 and earlier)
 */
export const PULSE_EXTENSIONS = ['.pls', '.pulse'];

/**
 * Check if a file path has a Pulse extension
 *
 * @param {string} filePath - Path to check
 * @returns {boolean} True if file has .pls or .pulse extension
 */
export function isPulseFile(filePath) {
  return PULSE_EXTENSIONS.some(ext => filePath.endsWith(ext));
}

/**
 * Normalize a Pulse file path by checking for both extensions
 * If both .pls and .pulse exist, .pls takes priority
 *
 * @param {string} filePath - Path with or without extension
 * @returns {string|null} Normalized path, or null if not found
 */
export function resolvePulseFile(filePath) {
  // If path already has extension and exists, use it
  if (isPulseFile(filePath) && existsSync(filePath)) {
    return filePath;
  }

  // If path has no extension, try .pls first, then .pulse
  if (!PULSE_EXTENSIONS.some(ext => filePath.endsWith(ext))) {
    for (const ext of PULSE_EXTENSIONS) {
...
export async function compileFile(filePath, options = {}) {
  ...
  if (legacyBackend) {
    ...
    const { dirname: pathDirname, relative: relativePath } = await import('node:path');
    ...
    // Compute runtime import path as absolute file:// URL
    // This ensures imports work regardless of where the temp file is created
    const runtimePath = fileURLToPath(new URL('../../runtime/index.js', import.meta.url));
    const runtimeImportPath = `file://${runtimePath}`;

    js = emitJS(optimized, { runtimeImportPath });
