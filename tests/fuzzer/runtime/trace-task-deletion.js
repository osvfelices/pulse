/**
 * Trace when the leaked task gets deleted
 */
...
async function trace() {
  ...
  // Intercept allTasks.delete
  const originalAllTasks = scheduler.allTasks;
  const deletedTasks = [];
  scheduler.allTasks = new Proxy(originalAllTasks, {
    get(target, prop) {
      if (prop === 'delete') {
        return (key) => {
          const task = target.get(key);
          if (task && task !== scheduler.rootTask) {
            ...
            console.log(`\n[allTasks.delete] Non-root task deleted:`);
            console.log(`  state=${task.state}, started=${task.started}`);
            console.log(`  Stack:\n${stack}`);
          }
  ...
  console.log(`Total non-root tasks deleted: ${deletedTasks.length}`);
}
